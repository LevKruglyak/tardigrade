...
layout(set = 0, binding = 1) buffer PositionMass {
    vec4 data[];
} pos_mass;
...
layout(set = 0, binding = 3) buffer Acceleration {
    vec4 data[];
} acc;
...
vec3 calculate_accel(vec3 target, vec3 position) {
    ...
    return sd.G * (target - position) / pow(dist2 + sd.softening, 1.5);
}
